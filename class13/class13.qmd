...
Today we will perform on RNASeq analysis on the effects of dexamethasone (hereafter "dex"), a common steroid, on airway smooth muscle (ASM) cell lines.

## Data Import

We need two things for this analysis:

-**countData**: a table with genes as rows and samples/experiments as colums,
...
We need to check that metadata matches the samples in our count data.
...
We have 4 replicates per condition ("control" and "treated"). We want to compare the control vs the treated to see whcih genes expression levels change when we have the drug present.
...
- Step 1. Find which columns in `counts` corrspond to "control" samples.
...
If we consider "treated"/"control" counts we will get a number that tells us the chnage.
...
Typically we would not consider zer count genes - as we have no data about them and they should be excluded from further consideration. These lead to "funky" log2 fold change values (e.g. divide by zero errors etc.)
...
```{r}
ggplot(res) + aes(x = log2FoldChange, y = -log(padj)) + geom_point(col = mycols) + geom_vline(xintercept = c(-2,+2), col="red") + geom_hline(yintercept = -log(0.05), col="red")
```
...
```{r}
write.csv(res, file="results_annoted.csv")
```
...
To run our pathway analysis we will use **gage()** function. It wants to main inputs: a vector of importance (in our case the log2 fold change vlaues); and the gene sets to check overlap for.
