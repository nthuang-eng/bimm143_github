...
```{r}
protein_prop <- stats["Protein (only)", "Total"] / n.sums["Total"]
round(protein_prop, 2)
```

> Q3: Type HIV in the PDB website search box on the home page and determine how many HIV-1 protease structures are in the current PDB?

4,713 Polymer Entities
...
> Q. One final image showing catalytic APS 25 as ball and stick and the all-important active
...
Now we can use the `install()` function from this packahe like this: `BiocManager::install("msa")`
...
we can run functions like `rsmd()`,  `rmsf()` and the best `pca()`
```{r}
pc.xray <- pca(pdbs)
plot(pc.xray)
```
